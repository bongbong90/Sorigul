...
import { api, getUserMessage, type RuntimeSettings, type ShutdownState } from '../api/client'
...
export function SettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      await api.health()
      const [saved, state] = await Promise.all([api.settings(), api.shutdown()])
      setSettings(saved); setShutdown(state); setBackend('CONNECTED'); setMessage('저장된 설정을 불러왔습니다.')
    ...
    <div className="feature-page settings-page">
      ...
      <div className="settings-grid">
        ...
        <Card className="settings-card"><div className="settings-card-heading"><MonitorCog aria-hidden="true" /><div><h3>Tray</h3><p>창을 닫을 때 Desktop runtime이 소비할 동작입니다.</p></div></div>
          ...
          <div className="setting-note"><Badge tone={settings.close_behavior === 'tray' ? 'done' : 'waiting'}>{settings.close_behavior === 'tray' ? 'Tray로 숨김' : '앱 종료'}</Badge><span>현재는 저장된 Desktop contract이며 OS 동작은 실행하지 않습니다.</span></div>
        </Card>
